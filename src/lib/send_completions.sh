...
send_completions() {
  ...
  echo $'      while read -r; do COMPREPLY+=( "$REPLY" ); done < <( compgen -W "$(_chronom-cli_completions_filter "--asm-ro-user-access-key --asm-ro-user-secret-key --asm-rw-user-access-key --asm-rw-user-secret-key --auto-validate --chronom-auth-id --chronom-namespace --chronom-readonly-username --chronom-registry-name --chronom-registry-username --chronom-version --create-rsa-key --dns-record --help --key-pair-name --max-nodes --max-nodes-large --min-nodes --name --node-type --node-type-large --region --ro-role-arn --ro-user-access-key --ro-user-secret-key --setup-calico-cni --skip-certificate-setup --skip-gp3-setup --skip-ingress-setup --version -d -h -n -r -v")" -- "$cur" )'
  echo $'      ;;'
  echo $''
