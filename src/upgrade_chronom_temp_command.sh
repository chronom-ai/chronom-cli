...
# authClientSecret=$(kubectl get secret auth-chronom-chronom-secret -n chronom -o jsonpath='{.data.AUTH_CLIENT_SECRET}' | base64 --decode)
...
helm upgrade -n "$namespace" chronom "oci://${registryAddress}/helm/chronom" --version "$version" -f ./values.yaml --set "initRegion=$region" --set "secretRegion=$region" --set-json="backend=$rwAccessKey" --set-json "awsscanner=$roAccessKey" #--set "auth.clientId=$chronomAuthId" --set "auth.clientSecret=$authClientSecret" --set "dnsRecord=$dnsRecord" --set "imageCredentials.username=$registryUsername" --set "imageCredentials.password=$registryPassword"
